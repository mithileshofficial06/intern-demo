  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        notify.warning('Fix your JSON syntax before generating', 'INVALID JSON')
        setError('INVALID JSON — FIX YOUR SYNTAX')
        return
      }
      const validationResult = safeValidateConfig(parsedJson)
      if (!validationResult.success) {
        notify.warning(validationResult.error, 'VALIDATION ERROR')
        setError(validationResult.error.toUpperCase())
        return
      }
      const toastId = notify.loading('GENERATING APP...')
      const response = await fetch('/api/runtime/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(parsedJson),
      })
      notify.dismiss(toastId as string | number)
      if (!response.ok) {
        notify.error('Failed to register app — try again', 'APP FAILED')
        setError('FAILED TO REGISTER APP')
        return
      }
      const { appId, name } = await response.json()
      notify.success(`"${name}" is ready!`, 'APP CREATED')
      router.push(`/${appId}`)
    } catch (err) {
      notify.error('Unexpected error occurred', 'SYSTEM ERROR')
